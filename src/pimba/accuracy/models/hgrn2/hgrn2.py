...
class HGRN2Attention(nn.Module):
    def __init__(
        ...
        if expand_ratio is None and num_heads is not None:
            ...
            raise RuntimeError(
                "One of `expand_ratio` or `num_heads` should be provided."
            )
        ...
        assert self.forget_dim % num_heads == 0, (
            f"forget dim must be divisible by num_heads of {num_heads}"
        )
        assert self.input_dim % num_heads == 0, (
            f"input dim must be divisible by num_heads of {num_heads}"
        )
        ...
        if use_short_conv:
            ...
            self.q_conv1d = ShortConvolution(
                self.forget_dim, conv_size, activation=None
            )
