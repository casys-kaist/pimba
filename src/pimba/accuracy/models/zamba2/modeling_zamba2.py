...
class Zamba2MambaMixer(nn.Module):
    ...
    def cuda_kernels_forward(
        ...
        _dt, _dA = fused_a_discretization(time_step, self.dt_bias, A)
        _dA = rearrange(_dA, "b l h -> b h l")
        _dA = repeat(_dA, "b h l -> b h l n", n=self.ssm_state_size)
        _x = rearrange(hidden_states, "b l (h p) -> b h l p", p=self.head_dim)
        _B = B.reshape(batch_size, seq_len, self.n_groups, 1, -1)
        _B = _B.expand(
            batch_size,
            seq_len,
            self.n_groups,
        ...
        _B = _B.reshape(batch_size, seq_len, -1, _B.shape[-1])
        _dB = torch.einsum("blh,blhn->bhln", _dt, _B)
        _C = C.reshape(batch_size, seq_len, self.n_groups, 1, -1)
        _C = _C.expand(
            batch_size,
            seq_len,
            self.n_groups,
        ...
        _C = _C.reshape(batch_size, seq_len, -1, _C.shape[-1])
        _C = rearrange(_C, "b l h n -> b h l n")
        state = original_input.new_zeros(
            batch_size, _C.shape[1], _x.shape[-1], _dB.shape[-1]
        )
        _y, _ = state_update(_C, _dB, _x, _dA, None, state)
        _y = fused_y_update(_x, _y, self.D)
        scan_output = rearrange(_y, "b h l n -> b l (h n)")
