...
class MultiScaleRetention(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.decay = 1 - torch.pow(
            2,
            -5 - torch.arange(0, num_heads, dtype=torch.float) * 1.0,
        ).to(configs.DEVICE)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.num_kv_groups > 1:
            k = repeat(
                k, "b t h d -> b (h g) t d", h=self.num_kv_heads, g=self.num_kv_groups
            )
            v = repeat(
                v, "b t (h d) -> b (h g) t d", h=self.num_kv_heads, g=self.num_kv_groups
            )
        else:
            k, v = (
                rearrange(k, "b t h d -> b h t d"),
                rearrange(v, "b t (h d) -> b h t d", h=self.num_kv_heads),
            )

        state = last_state[-1] if use_cache else None
        gk = self.decay[..., None, None].expand_as(k)
        scale = q.shape[-1] ** -0.5
        q = q * scale

        o, recurrent_state = state_update(
            q,
            k,
            v,
            gk,
