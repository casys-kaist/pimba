...
def draw_figure12():
    ...
    fig, ax = plt.subplots(figsize=(20, 2.5))  # type: ignore
    ...
    ax.set_xticklabels([])
    ax.tick_params("x", which="major", length=70, width=line_width)
    ax.tick_params("x", which="minor", length=12, width=line_width)
    ax.set_yticklabels([])

    fig.savefig(
        get_output_path("figure12"), format="pdf", bbox_inches="tight", pad_inches=0.025
    )
