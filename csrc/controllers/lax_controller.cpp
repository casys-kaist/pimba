...
class LAXController final : public IDRAMController, public Implementation {
    RAMULATOR_REGISTER_IMPLEMENTATION(IDRAMController, LAXController,
                                      "LAXController", "LAXController");

   private:
    std::deque<Request> pending;  // A queue for read requests that are about to
                                  // finish (callback after RL)

    ReqBuffer m_active_buffer;    // Buffer for requests being served. This has
   ...
   public:
    ...
    void tick() override {
        ...
        if (request_found) {
            ...
            // If we are issuing the last command, set depart clock cycle and
        ...
        if (pending.size()) {
            ...
            if (req.depart <= m_clk) {
                ...
                if (req.depart - req.arrive > 1) {
                    // Check if this requests accesses the DRAM or is being
                    // forwarded.
                    // TODO add the stats back
                }

                if (req.callback) {
                    // If the request comes from outside (e.g., processor), call
                    // its callback
                    req.callback(req);
                }
    ...
    bool schedule_request(ReqBuffer::iterator &req_it, ReqBuffer *&req_buffer) {
        ...
        if (!request_found) {
            ...
            if (m_priority_buffer.size() != 0) {
                ...
                req_it->command = m_dram->get_preq_command(
                    req_it->final_command, req_it->addr_vec);
            ...
            // 2.2.1    If no request to be scheduled in the priority buffer,
            // check the read and write buffers.
            if (!request_found) {
        ...
        // 2.3 If we find a request to schedule, we need to check if it will
        // close an opened row in the active buffer.
        if (request_found) {
            if (m_dram->m_command_meta(req_it->command).is_closing) {
                ...
                std::vector<Addr_t> rowgroup(
                    ...
                    (req_it->addr_vec).begin() + row_group_end);
                for (auto _it = m_active_buffer.begin();
                     _it != m_active_buffer.end(); _it++) {
                    std::vector<Addr_t> _it_rowgroup(
                        _it->addr_vec.begin(),
                        _it->addr_vec.begin() + row_group_end);
                    if (rowgroup == _it_rowgroup) {
                        // Invalidate this scheduling outcome if we are to
                        // interrupt a request in the active buffer
                        request_found = false;
                    }
                }
            }
